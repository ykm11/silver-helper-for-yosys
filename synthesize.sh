...
# This file is downloaded from PROLEAD https://github.com/ChairImpSec/PROLEAD/tree/main/yosys/syn
...
elif [[ $1 == "VERILOG" || $1 == "verilog" || $1 == "Verilog" ]]; then
    # Run 1.) - 12.)
    yosys -p "read_verilog $2*.v;
    read_verilog -lib ${LIBRARY_PATH}custom_cells.v; 
    setattr -set keep_hierarchy 1;
    synth -top $3 -noabc; 
    dfflibmap -liberty ${LIBRARY_PATH}custom_cells.lib; 
    abc -liberty ${LIBRARY_PATH}custom_cells.lib -fast;
    opt_clean;
    stat -liberty ${LIBRARY_PATH}custom_cells.lib; 
    setattr -set keep_hierarchy 0; 
    flatten; opt_clean -purge; select $3;
    write_verilog -noattr -selected design.v;"
else
